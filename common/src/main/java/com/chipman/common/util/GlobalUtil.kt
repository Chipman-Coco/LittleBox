...
object GlobalUtil {
    ...
    val appPackage: String
        get() = BaseApp.appContext.packageName
     ...
     */
    val appName: String
        get() = BaseApp.appContext.resources.getString(BaseApp.appContext.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = BaseApp.appContext.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            BaseApp.appContext.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            BaseApp.appContext.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(BaseApp.appContext.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
...
//            CoroutineScope(Dispatchers.IO).launch { SpUtil.putString("uuid", uuid) }
     ...
     */
    fun getString(resId: Int): String = BaseApp.appContext.resources.getString(resId)
     ...
     * @return 字符串资源id对应的字符串内容。
     */
    fun getDimension(resId: Int): Int = BaseApp.appContext.resources.getDimensionPixelOffset(resId)
     ...
     * @return 指定资源名的资源id。
     */
    fun getResourceId(name: String, type: String): Int = BaseApp.appContext.resources.getIdentifier(name, type, appPackage)
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = BaseApp.appContext.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            BaseApp.appContext.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = BaseApp.appContext.packageManager
