...
object DateUtil {
     ...
     * @return 格式化后的字符串
     */
    fun getDateFormatString(timestamp: Long, dateFormat: String = "yyyy-MM-dd HH:mm:ss"): String {
        val mTimeStamp = if (timestamp < 10f.pow(13)) {
            timestamp * 1000
        } else {
            timestamp
        }
        return SimpleDateFormat(dateFormat, Locale.CHINESE).format(Date(mTimeStamp))
    }
